...
import {
  ...
  ChevronRight, RefreshCw, Send, Check, Search, TrendingUp
} from 'lucide-react';
...
export default function CandidatePortal({ selectedDistrict, setSelectedDistrict }) {
  ...
  const [candidateName, setCandidateName] = useState('Rahul Kulkarni');
  const [education, setEducation] = useState('Diploma in Mechanical Engineering');
        ...
        <div className="card candidate-config-card">
          ...
          <div className="role-selector-pills">
            {roles.map(r => (
              <button
                key={r.id}
                className={`role-pill-btn ${selectedRole?.id === r.id ? 'active' : ''}`}
                onClick={() => handleRoleChange(r)}
              >
                <strong>{r.title}</strong>
                <span className="role-pill-sec">{r.sector_name.split(' ')[0]} • {r.salary_range}</span>
              </button>
            ))}
          </div>
          ...
          <div className="resume-paste-section">
            <label className="section-label">
              <Sparkles size={14} className="text-purple" /> Or Paste Resume / Experience Summary (Gemini NLP will extract skills):
            </label>
            <textarea
              className="textarea-field"
              rows={3}
              placeholder="e.g. Completed 1 year apprenticeship at Tata Motors. Worked on CNC lathe setup and basic wiring..."
              value={resumeText}
